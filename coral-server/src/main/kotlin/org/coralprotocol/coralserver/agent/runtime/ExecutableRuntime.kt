...
data class ExecutableRuntime(
    ...
    override fun spawn(
        ...
        logger.info { "Creating environment variables for agent ${params.agentName}" }
logger.info { "API URL: $apiUrl" }
logger.info { "MCP URL: $mcpUrl" }
        ...
        envsToSet.forEach { (key, value) ->
            logger.info { "Setting environment variable: $key=$value" }
        }

        val coralConnectionUrl = envsToSet["CORAL_CONNECTION_URL"]
if (coralConnectionUrl == null) {
    logger.error { "CORAL_CONNECTION_URL is missing from environment variables!" }
} else {
    logger.info { "CORAL_CONNECTION_URL: $coralConnectionUrl" }
}
